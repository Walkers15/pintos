...
#endif
	/*
	thread_current()->child_status = 2; // Parent에게 내 작업이 끝났음을 전달
	while (thread_current()->child_status != 3) {
		// DELETE DONE이 될때까지 busy waiting 하면서 대기
		// printf("부모가 날 처리하기를 대기중입니다...\n");
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  intr_set_level (old_level);
	...
	// Child List 초기화 (처음 시작시에도 실행되도록 init_thread에 추가해야 함)
	list_init(&(t->child_list));

	// 사용하는 semaphore 초기화
	sema_init(&t->wait, 0);
	sema_init(&t->post_process, 0);
	sema_init(&t->load, 0);
